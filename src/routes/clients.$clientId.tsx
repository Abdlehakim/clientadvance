      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <Card className="p-5 shadow-card lg:col-span-1">
          ...
          <dl className="space-y-3 text-sm">
            <div><dt className="text-muted-foreground">Téléphone</dt><dd className="font-medium">{client.telephone || "—"}</dd></div>
            <div><dt className="text-muted-foreground">Email</dt><dd className="font-medium">{client.email || "—"}</dd></div>
            <div><dt className="text-muted-foreground">CIN</dt><dd className="font-medium">{client.cin || "—"}</dd></div>
            <div><dt className="text-muted-foreground">Adresse</dt><dd className="font-medium">{client.adresse || "—"}</dd></div>
            <div><dt className="text-muted-foreground">Créé par</dt><dd className="font-medium">{client.created_by}</dd></div>
          </dl>
        </Card>
        ...
        <Card className="p-5 shadow-card lg:col-span-2">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Button size="sm" onClick={() => setOpen(true)}><Plus className="mr-2 h-4 w-4" /> Ajouter un paiement</Button>
          </div>
          <div className="overflow-x-auto rounded-md border">
            <Table>
              ...
              <TableBody>
                {payments.length === 0 ? (
                  <TableRow><TableCell colSpan={5} className="h-20 text-center text-muted-foreground">Aucun paiement.</TableCell></TableRow>
                ) : payments.map((p) => (
                  <TableRow key={p.id}>
                    <TableCell>{formatDateFR(p.date_paiement)}</TableCell>
                    <TableCell>{p.heure_paiement}</TableCell>
                    <TableCell className="font-medium">{formatTND(p.montant)}</TableCell>
